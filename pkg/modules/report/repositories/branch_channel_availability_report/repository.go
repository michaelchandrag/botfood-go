...
type Repository interface {
	...
	FindAll(filter Filter) (bcReport []entities.BranchChannelAvailabilityReport, err error)
}
...
func (r *repository) FindAll(filter Filter) (bcReport []entities.BranchChannelAvailabilityReport, err error) {
	queryBuilder := getQueryBuilder()
	queryWhere := generateFilter(filter)
	formattedQuery := queryBuilder + queryWhere + addSort(filter)
	err = r.db.GetDB().Select(&bcReport, formattedQuery)
	if err != nil {
		fmt.Println(err)
	}
	return bcReport, err
