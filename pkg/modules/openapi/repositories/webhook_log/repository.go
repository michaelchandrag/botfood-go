...
func (r *repository) Create(wl entities.WebhookLog) (newWl entities.WebhookLog, err error) {
	query := `
		INSERT INTO webhook_logs
			(brand_id, request_url, request_body, response_body, created_at, updated_at, deleted_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)`
	dt := time.Now()
	now := dt.Format("2006-01-02 15:04:05")
	wl.CreatedAt = &now
	wl.UpdatedAt = &now
	action := r.db.GetDB().MustExec(query, wl.BrandID, wl.RequestURL, wl.RequestBody, wl.ResponseBody, wl.CreatedAt, wl.UpdatedAt, nil)
